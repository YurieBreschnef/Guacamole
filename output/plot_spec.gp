...
do for [i=SINDEX:EINDEX] {
   ...
   set output './output/visual/spec_inspect_combo/'.i.'.png'
   ...
   set tics nomirror out scale 0.75
    ...
    set yrange [0.0:LY]
  	set title ' u_x '
   	plot './output/data/u/'.i.'.u.dat' using 1:2:3 with image notitle

    set xrange [0.0:XDIM]
    set yrange [0.0:YDIM]
     set cbrange[MINSPEC:MAXSPEC]
  	set title 'ln[Re(F(u_x))] '
   	plot './output/data/u_f/'.i.'.u_f.dat' using 1:2:3 with image notitle
  	set title 'ln[imag(F(u_x))] '
   	plot './output/data/u_f/'.i.'.u_f.dat' using 1:2:4 with image notitle
    ...
    set cbrange[MINSPEC:MAXSPEC]
  	set title 'ln[Re(F(u_y))] '
   	plot './output/data/u_f/'.i.'.u_f.dat' using 1:2:5 with image notitle
  	set title 'ln[imag(F(u_y))] '
   	plot './output/data/u_f/'.i.'.u_f.dat' using 1:2:6 with image notitle
    ...
    plot './output/data/k_spec/'.i.'.spec.dat' using 2:3 with lines notitle
    unset multiplot
}
