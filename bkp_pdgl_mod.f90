...
function calc_buo(u_f,u,in_field,diff,buo,strat,t)
  ...
  real(kind = rp),intent(in)                                   :: diff,buo,strat,t ! parameters
   ...
   calc_buo= calc_buo + buo_strat(u_f,u,in_field,strat)      !BACKGROUND STRATIFICATION
   calc_buo= dealiase_field(fc)
end function
...
function buo_adv(u_f,u,in_field,t)
  ...
  call transform(in_field%val,state%s_dummy%val,-1,shearing,t)
  ...
  !$omp do
    do i=0,xdim-1
      do j=0,ydim-1
      !                                            chem_f* u          (realspace)
      state%dummy%val(i,j,1) = state%s_dummy%val(i,j)*u%val(i,j,1)
      state%dummy%val(i,j,2) = state%s_dummy%val(i,j)*u%val(i,j,2)
      end do
    end do
  !$omp end do
  !$omp end parallel
...
function buo_diff(in_field,diff)
  ...
  !$omp do
    do i=0,xdim-1
      do j=0,ydim-1
        fc_diff(i,j)  = diff*( state%iki_bar_sqr%val(i,j)*in_field(i,j))
        !Note that the minus sign is intrinsicly included by the squared (ikx**2 + iky**2)
      end do
    end do
  !$omp end do
